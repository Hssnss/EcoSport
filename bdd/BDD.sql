...
create view vueCommandes as(
    select c.*, ca.idarticle, ca.quantite from commande c, commandearticle ca where c.idcommande=ca.idcommande ORDER BY c.idcommande
);
...
CREATE VIEW vue_ContenuCommande AS(
        SELECT c.idcommande, c.idarticle, a.Nom, c.quantite, c.prixunitaire, a.NomImage, a.Description, a.prix
        FROM contenucommande c, article a
